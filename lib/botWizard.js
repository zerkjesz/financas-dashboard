...
import { formatMoney, formatDate, formatDueDate, NO_DUE_DATE_LABEL } from "./formatMoney.js";
import { serializeMoney, subtractMoney } from "./money.js";
import { resolveCurrentBillSafely, applyObservedTotal } from "./cardBillCalculator.js";
...
// Fase 8.0.1 — os prompts de vencimento prometem "12/03", mas resolveDate (linguagem natural legada) só
// entende "dia 25", "amanhã" etc. e devolvia null pra DD/MM — o usuário ficava preso em "Não entendi a
// data". Tenta primeiro o parser estrito DD/MM[/AAAA] (mesmo dos outros wizards) e cai no natural.
async function resolveDueDateText(text) {
  const strict = parseWizardDateText(text);
...
async function askFaturaPagarValor(session, client) {
  let hint = "";
  try {
    const persistedBill = await resolveCurrentBillSafely(session.data.cardId, { client });
    const hintCard = await client.card.findUnique({ where: { id: session.data.cardId } });
    const bill = await applyObservedTotal(hintCard, { ...persistedBill, isPersisted: true }, { client }); // total autoritativo (observado, se houver)
...
async function askCompromissoVencimento(session, client) {
  await ask(session, "Vence quando? (ex: \"dia 25\", \"12/03\")\n\nSe não tem prazo definido, toque no botão — o Norte nunca inventa uma data.", { replyMarkup: withNav([[{ text: `⏳ ${NO_DUE_DATE_LABEL}`, data: "skip:duedate" }]]) }, client);
}

async function finishCompromissoPreview(session, client) {
  const summary = ["Confirma?", "", `📌 ${session.data.description}`, formatMoney(session.data.amount), session.data.dueDateISO ? `Vence: ${formatDate(new Date(session.data.dueDateISO))}` : `Prazo: ${NO_DUE_DATE_LABEL}`].join("\n");
  await updateStep(session, "confirmar", {}, { client });
  await ask(session, summary, { replyMarkup: CONFIRM_KEYBOARD }, client);
}

async function commitCompromisso(session, client) {
  const { reply } = await commitBotIntent("create_confirmed_commitment", { description: session.data.description, amount: session.data.amount, dueDate: session.data.dueDateISO ?? null }, { source: "telegram", client });
  return reply;
}
...
export async function handleWizardCallback(chatId, callbackData, { client = prisma } = {}) {
  ...
  if (session.step === "vencimento" && callbackData.startsWith("due:")) {
    ...
    const dueDate = await resolveDueDateText(value);
  ...
  if (session.step === "campo" && session.flow === "compromisso_editar" && callbackData.startsWith("cfield:")) {
    ...
    if (field === "amount") await ask(session, "Novo valor?", undefined, client);
    else await ask(session, "Nova data de vencimento? (ex: \"dia 25\", \"12/03\")", { replyMarkup: withNav([[{ text: `⏳ ${NO_DUE_DATE_LABEL}`, data: "skip:duedate" }]]) }, client);
    return;
  }
  // Fase 8.0.1 — "sem prazo definido": no cadastro, segue pro preview com dueDate=null; na edição,
  // LIMPA o prazo (dueDate=null explícito). Nunca uma data fictícia.
  if (callbackData === "skip:duedate" && session.flow === "compromisso" && session.step === "vencimento_texto") {
    return finishCompromissoPreview(await updateStep(session, "confirmar", { dueDateISO: null }, { client }), client);
  }
  if (callbackData === "skip:duedate" && session.flow === "compromisso_editar" && session.step === "novo_valor" && session.data.field === "dueDate") {
    const { getCommitment } = await import("./commitments.js");
    const commitment = await getCommitment(session.data.commitmentId, { client });
    await updateStep(session, "confirmar", { newDueDate: null }, { client });
    await ask(session, ["Confirma?", "", `📌 ${commitment.description}`, `Vencimento: ${formatDueDate(commitment.dueDate)} -> ${NO_DUE_DATE_LABEL}`].join("\n"), { replyMarkup: CONFIRM_KEYBOARD }, client);
    return;
  }
...
export async function handleWizardText(chatId, text, { client = prisma } = {}) {
  ...
  if (session.flow === "nova_conta" && session.step === "vencimento_texto") {
    const dueDate = await resolveDueDateText(trimmed);
    await updateStep(session, session.step, { pendingDate: dueDate ? dueDate.toISOString() : null }, { client });
    await finishNovaContaDate(session, client);
    return true;
  }
  ...
  if (session.step === "valor") {
    ...
    } else if (session.flow === "compromisso") {
      // Fase 8.0.1 — achado real: este ramo faltava (o valor digitado era engolido em silêncio e o
      // wizard "Novo compromisso" nunca avançava); o ramo específico abaixo era inalcançável.
      await askCompromissoVencimento(await updateStep(session, "vencimento_texto", { amount }, { client }), client);
  ...
  if (session.flow === "compromisso_editar" && session.step === "novo_valor") {
    ...
    await ask(session, ["Confirma?", "", `📌 ${commitment.description}`, `Vencimento: ${formatDueDate(commitment.dueDate)} -> ${formatDate(dueDate)}`].join("\n"), { replyMarkup: CONFIRM_KEYBOARD }, client);
    return true;
  }
