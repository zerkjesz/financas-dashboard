...
export async function buildProjectionScenario(scenario, { horizonDays = DEFAULT_HORIZON_DAYS, now = new Date(), accounts: accountsIn, extraEvents = [], client = prisma } = {}) {
  ...
  // Fase 5.3E — eventos hipotéticos do simulador (sempre [] em qualquer
  // chamada real do produto/bot hoje). Entram DEPOIS de todos os eventos reais
  // e da lógica de contingência acima, na MESMA lista consumida por
  // buildTimeline — nenhuma segunda passada de projeção, nenhuma duplicação de
  // regra. `extraEvents` nunca é lido de banco aqui; quem monta o array
  // (lib/simulation/financialSimulator.js) é responsável por construir cada
  // item a partir do cenário hipotético informado pelo usuário.
