...
// ============================================================================
// Fase 4.1, itens 12-16 — motor de projeção V2 (base/expected/stress).
// lib/cashFlowProjection.js (V1) foi removido na Fase 5.4F — este arquivo é
// o único motor de projeção do produto agora.
//
...
// Fase 5.4F — relocado de lib/cashFlowProjection.js (V1, removido) na
// remoção do V1: era o único export daquele arquivo com um caller real vivo
// (app/api/cash-flow/route.js, só como constante de validação de query
// param — nunca chamava nenhuma função de cálculo V1). Puro array, zero
// lógica financeira.
...
async function collectOutflowEvents({ now, horizonEnd, client = prisma }) {
  const events = [];
  ...
  // Fase 9.1.1 — contas da casa pendentes com valor conhecido: vencimento conhecido => na data (ou hoje,
  // se já venceu); sem vencimento => fim da competência (sabemos que sai neste mês — só a data é
  // aproximada, o valor nunca). Variável sem valor NÃO entra (nunca vira zero fictício).
  const house = await getHouseBillObligations({ now, horizonEnd, client });
  for (const item of house.items) {
    let date;
    if (item.dueDate) date = item.dueDate >= now ? item.dueDate : now;
    else {
      const [y, m] = item.cycleMonth.split("-").map(Number);
      const eom = new Date(Date.UTC(y, m, 0));
      date = eom >= now ? eom : now;
    }
    if (date > horizonEnd) continue;
    events.push({ date, label: item.description, kind: "bill", amount: multiplyMoney(item.amount, -1) });
  }

  const bills = await client.bill.findMany({ where: { status: { in: ["pending", "overdue"] }, dueDate: { gte: now, lte: horizonEnd } } });
  for (const bill of bills) {
  ...
  const externalInstallments = await client.externalInstallment.findMany({
  ...
  const commitments = await client.confirmedCommitment.findMany({
...
// Fase 5.3E — `extraEvents` opcional (default: []), aditivo: permite ao
// simulador (lib/simulation/financialSimulator.js) injetar UM evento
// hipotético (gasto/compra/parcela simulada) na MESMA timeline, reaproveitando
// 100% desta engine (nunca uma segunda implementação de projeção). Nenhum
// call-site existente muda de comportamento (default = nenhum evento extra).
// Cada item de `extraEvents`: { date, label, kind, amount } — mesma forma dos
// eventos internos (`amount` negativo = saída).
// Fase 10 — a COLETA de insumos (I/O: renda, saídas confirmadas, contingências) foi separada da MONTAGEM
// (pura: ordena eventos, calcula saldo corrente e checkpoints). Assim o simulador pode coletar UMA vez e
// avaliar dezenas de compras hipotéticas em memória (busca da capacidade segura) usando exatamente a mesma
// montagem — nenhuma segunda implementação de projeção. Comportamento sem `prepared` é idêntico ao anterior.
export async function prepareProjectionInputs(scenario, { horizonDays = DEFAULT_HORIZON_DAYS, now = new Date(), accounts: accountsIn, client = prisma } = {}) {
  ...
  const [{ events: incomeEvents, overdueExpectedIncome }, outflowEvents] = await Promise.all([
    ...
    collectOutflowEvents({ now, horizonEnd, client }),
  ]);
  ...
  return { scenario, horizonDays, now, startingCash, protectedMoney, events, contingencyUndated, riskExposure, overdueExpectedIncome };
}

// buildProjectionScenario("BASE" | "EXPECTED" | "STRESS", { horizonDays, now, accounts })
// Fase 5.3E — `extraEvents` opcional (default: []), aditivo: permite ao
// simulador (lib/simulation/financialSimulator.js) injetar eventos
// hipotéticos (gasto/compra/parcela simulada) na MESMA timeline, reaproveitando
// 100% desta engine (nunca uma segunda implementação de projeção). Nenhum
// call-site existente muda de comportamento (default = nenhum evento extra).
// Cada item de `extraEvents`: { date, label, kind, amount } — mesma forma dos
// eventos internos (`amount` negativo = saída).
// Fase 10 — `prepared` opcional (retorno de prepareProjectionInputs): pula toda a coleta de I/O.
export async function buildProjectionScenario(scenario, { horizonDays = DEFAULT_HORIZON_DAYS, now = new Date(), accounts: accountsIn, extraEvents = [], client = prisma, prepared } = {}) {
  const prep = prepared ?? (await prepareProjectionInputs(scenario, { horizonDays, now, accounts: accountsIn, client }));
  const { startingCash, protectedMoney, contingencyUndated, riskExposure, overdueExpectedIncome } = prep;
  const usedNow = prep.now ?? now;

  // Fase 5.3E — eventos hipotéticos do simulador (sempre [] em qualquer chamada real do produto/bot
  // hoje). Entram DEPOIS de todos os eventos reais, na MESMA lista consumida por buildTimeline.
  const events = extraEvents.length > 0 ? [...prep.events, ...extraEvents] : prep.events;

  const timeline = buildTimeline({ startingCash, events });

  const checkpoints = {};
  for (const { key, days } of CHECKPOINTS) {
    const checkpointDate = addDays(usedNow, days);
  ...
  return {
    scenario,
    horizonDays: prep.horizonDays ?? horizonDays,
    startingCash,
    protectedMoney,
    timeline,
    checkpoints,
