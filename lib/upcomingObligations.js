...
import { applyObservedTotal } from "./cardBillCalculator.js";
...
export async function listUpcomingObligations({ withinDays = 45, cards: cardsIn, productSnapshot } = {}) {
  ...
  // Fase 9.1 — GET/leitura NÃO materializa Bill (antes: ensureUpcomingRecurringBills() escrevia aqui).
  ...
  const items = [];
  // Fase 9.1.1 — contas da casa pendentes com valor e vencimento conhecidos (as sem vencimento/valor não são
  // evento datado); Bill legada já coberta por elas é ignorada abaixo (sem double count).
  const house = await getHouseBillObligations({ now, horizonEnd: horizon });
  for (const it of house.items) {
    if (!it.dueDate || it.dueDate > horizon) continue;
    items.push({ name: it.description, amount: multiplyMoney(it.amount, -1), date: it.dueDate, status: it.overdue ? "atrasada" : "pendente", kind: "bill" });
  }
  for (const bill of bills) {
    const hk = billHouseKey(bill);
    if (hk && house.handled.has(hk)) continue;
    if (!bill.dueDate) continue; // sem vencimento informado: não é um evento datado
    items.push({
  ...
  const effectiveByCard = await Promise.all(
    cards.map((card, i) => Promise.all(cardBillsByCard[i].map((bill) => applyObservedTotal(card, { ...bill, isPersisted: true }))))
  );
  cards.forEach((card, i) => {
    for (const bill of effectiveByCard[i]) {
