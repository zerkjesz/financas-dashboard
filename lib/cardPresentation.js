...
// Fase 5.4D.1.1 — BUG REAL corrigido (achado ao vivo no stress test de
// valor grande, R$999.999,99): a formatação manual (`toFixed(2).replace`)
// nunca colocava separador de milhar — "R$ 999999,98" em vez de
// "R$ 999.999,98", inconsistente com TODO o resto da página (que usa
// formatMoney em tudo). Invisível no dado real (o gap real nunca passou de
// R$656,37, abaixo de 1000) — exatamente o tipo de bug que só aparece com
// dado extremo, por isso o stress test.
export function detailGapLabel(bill) {
  if (!bill.hasDetailGap) return null;
  return `${formatMoney(bill.undetailedAmount)} ainda sem detalhamento individual`;
}
