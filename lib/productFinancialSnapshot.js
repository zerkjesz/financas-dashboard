...
export async function buildProductFinancialSnapshot({ client = prisma, now = new Date() } = {}) {
  ...
  // ---- houseBills (Fase 9.1.1) — contas da casa já DENTRO de currentObligations.breakdown (type "Bill",
  // houseBill:true); aqui só o resumo + as que estão "aguardando valor" (nunca contadas como zero).
  const houseBills = {
    pendingKnownCount: engine.obligations.houseBills?.pendingKnownCount ?? 0,
    pendingKnownAmount: engine.obligations.houseBills?.pendingKnownAmount ?? ZERO,
    unpricedPendingBillsCount: engine.obligations.houseBills?.unpricedPendingBillsCount ?? 0,
    unpricedPendingBills: engine.obligations.houseBills?.unpricedPendingBills ?? [],
  };
  ...
  // ---- projectionSummary (Fase 5.4C) --------------------------------------
  // A Home precisa de um resumo compacto de 30/60/90 sem reimplementar
  // projeção nenhuma: engine.projections (lib/financialProjection.js, V2 —
  // base/expected/stress) já vem 100% pronta de buildFinancialEngineSummary,
  // só nunca tinha sido reexposta por este read model. Aqui só RECORTA os
  // checkpoints que a apresentação usa (day30/day60/day90) — nenhuma soma,
  // nenhum recálculo. BASE e STRESS ficam separados de propósito (item 31 —
  // nunca reduzir a um único número escondendo o cenário de risco).
  const projectionSummary = {
    horizonDays: engine.projections.base.horizonDays,
    base: {
      day30: engine.projections.base.checkpoints.day30.projectedCash,
      day60: engine.projections.base.checkpoints.day60.projectedCash,
      day90: engine.projections.base.checkpoints.day90.projectedCash,
    },
    stress: {
      day30: engine.projections.stress.checkpoints.day30.projectedCash,
      day60: engine.projections.stress.checkpoints.day60.projectedCash,
      day90: engine.projections.stress.checkpoints.day90.projectedCash,
    },
  };
