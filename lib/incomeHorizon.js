...
export async function getRealizedOccurrences(recurringRuleIds) {
...
export async function resolveNextExpectedIncomeFromDb({ now = new Date() } = {}) {
  const [recurringRules, accounts, settings] = await Promise.all([
    prisma.recurringRule.findMany({ where: { kind: "income", isActive: true } }),
    prisma.account.findMany(),
    getAppSettings(),
  ]);
  const realizedIncomes = await getRealizedOccurrences(recurringRules.map((r) => r.id));
...
export async function getNextIncomeInfo({ now = new Date() } = {}) {
  const horizon = await resolveNextExpectedIncomeFromDb({ now });
  let amount = null;
  if (horizon.recurringRuleId) {
    const rule = await prisma.recurringRule.findUnique({ where: { id: horizon.recurringRuleId } });
