...
import { buildBaseProjection, buildExpectedProjection, buildStressProjection, prepareProjectionInputs } from "../financialProjection.js";
...
async function buildCardOverlay({ client, now, card, installmentRows, realBills: realBillsIn }) {
  const realBills = realBillsIn ?? (await listCardBillsView(card.id, { now, client }));
...
// ============================================================================
// Fase 10 — CONTEXTO PRÉ-CARREGADO. Toda a parte cara (I/O) de uma simulação — baseline do motor, contas,
// insumos das 3 projeções, faturas reais e limite do cartão — não depende do cenário hipotético. Carregar
// UMA vez e passar `context` permite avaliar dezenas de compras (ex.: busca binária da capacidade segura
// em /cartoes) usando EXATAMENTE a mesma função, em memória. Sem `context`, comportamento idêntico ao anterior.
export async function prepareSimulationContext({ client = prisma, now = new Date(), horizonDays = DEFAULT_HORIZON_DAYS, cardId } = {}) {
  const [baselineEngine, accounts] = await Promise.all([buildFinancialEngineSummary({ now, horizonDays, client }), listAccountsWithBalances({ client })]);
  const [BASE, EXPECTED, STRESS] = await Promise.all(["BASE", "EXPECTED", "STRESS"].map((sc) => prepareProjectionInputs(sc, { horizonDays, now, accounts, client })));
  const ctx = { now, horizonDays, baselineEngine, accounts, prepared: { BASE, EXPECTED, STRESS }, cards: {}, cardBills: {}, cardLimits: {} };
  if (cardId) {
    const card = await client.card.findUnique({ where: { id: cardId } });
    if (card) {
      ctx.cards[card.id] = card;
      ctx.cardBills[card.id] = await listCardBillsView(card.id, { now, client });
      const [totalLimit, usedLimit] = await Promise.all([computeCardTotalLimit(card.id, { client }), computeCardUsedLimit(card.id, { client })]);
      ctx.cardLimits[card.id] = { totalLimit, usedLimit };
    }
  }
  return ctx;
}

export async function simulateFinancialScenario({ client = prisma, now = new Date(), horizonDays = DEFAULT_HORIZON_DAYS, scenario, context } = {}) {
  ...
  const [baselineEngine, accounts] = context
    ? [context.baselineEngine, context.accounts]
    : await Promise.all([buildFinancialEngineSummary({ now, horizonDays, client }), listAccountsWithBalances({ client })]);
  ...
  if (type === SIMULATION_SCENARIO_TYPE.CASH_EXPENSE_NOW) {
    ...
    const card = context?.cards?.[scenario.cardId] ?? (await client.card.findUnique({ where: { id: scenario.cardId } }));
    ...
    const cachedLimit = context?.cardLimits?.[card.id];
    const [totalLimit, usedLimit] = cachedLimit ? [cachedLimit.totalLimit, cachedLimit.usedLimit] : await Promise.all([computeCardTotalLimit(card.id, { client }), computeCardUsedLimit(card.id, { client })]);
    ...
    const overlay = await buildCardOverlay({ client, now, card, installmentRows: rows, realBills: context?.cardBills?.[card.id] });
  ...
  const [base, expected, stress] = await Promise.all([
    buildBaseProjection({ now, accounts, horizonDays, extraEvents, prepared: context?.prepared?.BASE }),
    buildExpectedProjection({ now, accounts, horizonDays, extraEvents, prepared: context?.prepared?.EXPECTED }),
    buildStressProjection({ now, accounts, horizonDays, extraEvents, prepared: context?.prepared?.STRESS }),
  ]);
  ...
  return {
    ...
    installmentSchedule,
    explanation,
    // Fase 9.1.1 — contas da casa: as pendentes com valor conhecido JÁ estão em baseline.currentHorizonObligations
    // (o cenário parte da mesma verdade do produto); as SEM valor não entram (nunca viram zero) e o
    // resultado avisa que a simulação não as inclui.
    houseBills: {
      includedPendingKnownCount: baselineEngine.obligations.houseBills?.pendingKnownCount ?? 0,
      includedPendingKnownAmount: baselineEngine.obligations.houseBills?.pendingKnownAmount ?? ZERO,
      unpricedPendingBillsCount: baselineEngine.obligations.houseBills?.unpricedPendingBillsCount ?? 0,
      unpricedPendingBills: baselineEngine.obligations.houseBills?.unpricedPendingBills ?? [],
      note: (baselineEngine.obligations.houseBills?.unpricedPendingBillsCount ?? 0) > 0
        ? `A simulação não inclui ${baselineEngine.obligations.houseBills.unpricedPendingBillsCount === 1 ? "1 conta" : `${baselineEngine.obligations.houseBills.unpricedPendingBillsCount} contas`} ainda sem valor: ${baselineEngine.obligations.houseBills.unpricedPendingBills.map((b) => b.name).join(", ")}.`
        : null,
    },
