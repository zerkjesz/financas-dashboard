...
// Fase 5.3C.2 — `client` opcional (default: `prisma`) em todas as funções
// deste arquivo, mesmo padrão aditivo já usado no resto do projeto: permite
// ler DENTRO da mesma transação Prisma do caminho de escrita do Telegram
// (lib/telegramUpdateHandler.js), pra consistência de snapshot. Nenhum
...
export async function resolvePaymentTarget(rawMessage, { client = prisma } = {}) {
  ...
  for (const rule of ACCOUNT_RULES) {
    if (rule.words.some((w) => includesWord(normalized, w))) {
      const account = await client.account.findUnique({ where: { slug: rule.slug } });
      if (account) return { type: "account", account };
    }
  }

  // sem menção explícita: cai na conta principal (única conta corrente real hoje)
  const fallback = await client.account.findFirst({ where: { type: "checking" }, orderBy: { createdAt: "asc" } });
  return fallback ? { type: "account", account: fallback } : { type: "account", account: null };
}

export async function getDefaultCard({ client = prisma } = {}) {
  return client.card.findFirst({ orderBy: { createdAt: "asc" } });
}

export async function getDefaultAccount({ client = prisma } = {}) {
  return client.account.findFirst({ where: { type: "checking" }, orderBy: { createdAt: "asc" } });
}

// Retorna as contas mencionadas na mensagem, na ordem em que aparecem (usado por transferências).
export async function resolveMentionedAccounts(rawMessage, { client = prisma } = {}) {
  const normalized = normalize(rawMessage);
  const found = [];
  for (const rule of ACCOUNT_RULES) {
  ...
  for (const f of found) {
    const account = await client.account.findUnique({ where: { slug: f.slug } });
