...
export async function resolveMentionedAccounts(rawMessage) {
  ...
  for (const rule of ACCOUNT_RULES) {
    for (const word of rule.words) {
      const idx = normalized.indexOf(normalize(word));
      if (idx !== -1) {
        found.push({ slug: rule.slug, index: idx });
        break;
      }
    }
  }
