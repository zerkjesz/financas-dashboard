...
export async function buildFinancialEngineSummary({ now = new Date(), horizonDays = 90, client = prisma } = {}) {
  ...
  const nextIncomeCommitment = await getNextIncomeCommitment({ nextIncome, client, now });
  ...
  const [base, expected, stress] = await Promise.all([
    buildBaseProjection({ horizonDays, now, accounts, client }),
    buildExpectedProjection({ horizonDays, now, accounts, client }),
    buildStressProjection({ horizonDays, now, accounts, client }),
  ]);
  ...
  return {
    ...
    obligations: {
      ...
      unfundedConfirmedCommitments,
      // Fase 9.1.1 — contas da casa: pendentes conhecidas (já dentro de currentHorizon) e as SEM valor.
      houseBills: obligations.houseBills,
    },
