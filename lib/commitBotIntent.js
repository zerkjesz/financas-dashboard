...
async function commitCreateConfirmedCommitment(data, source, client) {
  ...
  return { record: commitment, reply: `✅ Compromisso confirmado: "${commitment.description}" — ${formatMoney(serializeMoney(commitment.amount))}, ${commitment.dueDate ? `vence em ${formatDate(commitment.dueDate)}` : "sem prazo definido"}.` };
}
...
async function commitUpdateConfirmedCommitment(data, source, client) {
  ...
  return { record: updated, reply: `✅ Compromisso "${updated.description}" atualizado: ${formatMoney(serializeMoney(updated.amount))}, ${updated.dueDate ? `vence em ${formatDate(updated.dueDate)}` : "sem prazo definido"}.` };
}
