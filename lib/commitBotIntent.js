...
import { getOrCreateBill, payBill, anticipateBill } from "./cardBillCalculator.js";
...
async function commitInstallmentPurchase(data, source) {
  ...
  const installmentValue = Math.round((data.amount / count) * 100) / 100;
  ...
  return {
    record: purchase,
    reply: `✅ Compra parcelada registrada: ${formatMoney(data.amount)} em ${count}x de ${formatMoney(installmentValue)} no cartão ${card.name}, a partir de ${firstInstallmentMonth}.`,
  };
}
...
async function commitBillPayment(data, source) {
  ...
  if (data.billPaymentKind === "installment_anticipation") {
    const transfer = await anticipateBill(card.id, {
      amount: data.amount,
      description: data.description,
      source,
      rawMessage: data.rawMessage,
    ...
    return {
      record: transfer,
      reply: `✅ Antecipação registrada: ${formatMoney(data.amount)} no cartão ${card.name}. Isso reduz o limite usado sem quitar a fatura inteira.`,
    };
  }
...
async function commitLimitUpdate(data, source) {
  ...
  const newUsedLimit = Math.max(0, totalLimit - data.amount);
  const limitUpdate = await prisma.cardLimitUpdate.create({
    data: {
      cardId: card.id,
      newUsedLimit,
      reportedAvailable: data.amount,
      source,
      rawMessage: data.rawMessage,
    },
  });
...
async function commitCreateBill(data, source) {
  ...
  return {
    record: bill,
    reply: `✅ Conta a pagar criada: "${bill.description}" — ${formatMoney(bill.amount)}, vence em ${formatDate(bill.dueDate)}.`,
  };
}
...
async function commitPayBill(data, source) {
  ...
  return { record: expense, reply: `✅ Conta "${bill.description}" marcada como paga: ${formatMoney(expense.amount)}.` };
}
...
async function commitCreateCard(data, source) {
  ...
  return {
    record: card,
    reply: `✅ Cartão "${card.name}" criado: limite ${formatMoney(card.totalLimit)}, vencimento dia ${card.dueDay}${card.closingDay ? `, fechamento dia ${card.closingDay}` : ""}.`,
  };
}
...
async function commitCreateRecurringBill(data, source) {
  ...
  return {
    record: rule,
    reply: `✅ Conta recorrente "${rule.name}" criada: ${formatMoney(rule.amount)} todo dia ${rule.dayOfMonth}. Ela já vai aparecer nas próximas obrigações e no fluxo de caixa.`,
  };
}

async function commitCreateGoal(data, source) {
  const goal = await prisma.goal.create({ data: { name: data.goalName, targetAmount: data.targetAmount } });
  return { record: goal, reply: `✅ Meta "${goal.name}" criada: guardar ${formatMoney(goal.targetAmount)}.` };
}
...
async function commitAddToGoal(data, source) {
  ...
  return { record: goal, reply: `✅ Guardado mais ${formatMoney(data.amount)} pra meta "${goal.name}". Total: ${formatMoney(goal.savedAmount)}.` };
}
