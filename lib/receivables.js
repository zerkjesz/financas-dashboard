...
export async function listReceivables({ status, client = prisma } = {}) {
  return client.receivable.findMany({
...
// Fase 7D — `client` opcional (default: `prisma`), mesmo padrão aditivo do
// resto do projeto: quando chamado a partir de lib/telegramUpdateHandler.js
// (dentro da transação que já reivindica o update_id), passar `{client: tx}`
// evita abrir uma SEGUNDA transação aninhada (que quebraria a garantia de
// "tudo persiste ou nada persiste" pro update inteiro). Sem `client`
// explícito, mantém o comportamento original: abre sua própria transação
// (nenhum call-site pré-existente muda de comportamento).
export async function markReceivableReceived(receivableId, { accountId, occurredAt, description, category, confidence, client } = {}) {
...
// Fase 7D.1 — não existia antes (só create/list/markReceived/cancel);
// necessário pro "✏️ Editar" do menu determinístico. Mesmo padrão de
// updateCommitmentDetails/updateContingencyAmount: bloqueia edição depois
// que o dinheiro já virou real (RECEIVED) ou foi descartado (CANCELLED) —
// nunca reabre um recebível já resolvido por engano.
export async function updateReceivableDetails(receivableId, { amount, expectedDate } = {}, { client = prisma } = {}) {
  const receivable = await client.receivable.findUnique({ where: { id: receivableId } });
  if (!receivable) throw new Error("Receivable não encontrado");
  if (receivable.status === "RECEIVED" || receivable.status === "CANCELLED") {
    throw new Error(`Receivable ${receivable.status} não pode ser editado`);
  }
  const data = {};
  if (amount != null) data.amount = money(amount);
  if (expectedDate != null) data.expectedDate = new Date(expectedDate);
  if (Object.keys(data).length === 0) throw new Error("nada pra atualizar");
  return client.receivable.update({ where: { id: receivableId }, data });
}
