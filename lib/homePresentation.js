...
//
// Fase 5.4D — ACHADO REAL: ExternalInstallment não tem campo `description`
// próprio (a descrição vive no PLANO — `planDescription`, ver
// lib/freeMoney.js/engine.obligations items). Sem este caso, um item desse
// tipo caindo em `currentObligations.breakdown` (CURRENT_HORIZON_OBLIGATION
// com dueTiming=CALENDAR_DATE, ver lib/obligationClassifier.js) renderizaria
// com label vazio — tanto na Home quanto em /compromissos (achado ao vivo
// nesta fase, na seção "Sai da próxima renda"). Aditivo: nenhum dado real
// hoje passa por este caminho na Home (confirmado — breakdown atual só tem
// CardBill/Bill/ConfirmedCommitment), então não muda nada do que já está
// aprovado; só fecha um bug latente pro dia em que passar a ter.
export function obligationItemLabel(item) {
  if (item.type === "CardBill") return `Fatura ${item.cardName} (${item.cycleMonth})`;
  if (item.type === "ExternalInstallment") return `${item.planDescription} (parcela ${item.number})`;
...
// Fase 8.0.1 — compromisso real SEM prazo definido (dueDate null): a UI mostra
// "Sem prazo definido" em vez de esconder a data ou renderizar uma data inválida.
export const NO_DUE_DATE_TEXT = "Sem prazo definido";
export function obligationItemHasNoDueDate(item) {
  return item.type === "ConfirmedCommitment" && obligationItemDate(item) == null;
}

// Legenda de um item: sem prazo, "até a próxima renda" seria enganoso (não há data) — diz o que
// é de verdade: dinheiro já separado (FUNDED) e/ou sem prazo definido.
export function obligationItemCaption(item, classLabel) {
  if (!obligationItemHasNoDueDate(item)) return classLabel ?? null;
  return item.status === "FUNDED" ? `Dinheiro separado · ${NO_DUE_DATE_TEXT}` : NO_DUE_DATE_TEXT;
}
