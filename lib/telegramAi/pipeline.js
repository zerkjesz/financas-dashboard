...
export async function handleConversationalMessage(text, chatId, { client = prisma, now = new Date(), provider, rawMessage } = {}) {
  ...
  if (interpretation.kind === INTERPRETER_RESULT_KIND.PROVIDER_UNAVAILABLE) {
    logPipelineEvent("provider_unavailable", { chatHash, provider: provider?.name ?? null });
    return { kind: PIPELINE_RESULT_KIND.NO_PROVIDER };
  }
  if (interpretation.kind !== INTERPRETER_RESULT_KIND.OK) {
    // Provider configurado mas ESTA chamada falhou (timeout/erro/rate
    // limit/JSON inválido) — item 21: zero write, nunca cai pro parser
    // antigo (evitaria reinterpretar com uma lógica diferente e
    // potencialmente incompatível). `detail` aqui é sempre uma mensagem de
    // validação/erro genérica (nunca o prompt ou o valor de um campo do
    // usuário) — seguro pra logar. `usage`/`provider`/`model` — item 13:
    // observabilidade de custo, nunca conteúdo financeiro.
    logPipelineEvent("interpretation_failed", {
      chatHash,
      kind: interpretation.kind,
      provider: provider?.name ?? null,
      latencyMs: interpretation.latencyMs ?? null,
      detail: interpretation.detail ?? null,
      retryAfterSeconds: interpretation.retryAfterSeconds ?? null,
      usage: interpretation.usage ?? null,
    });
    return { kind: PIPELINE_RESULT_KIND.REPLY, reply: PROVIDER_UNAVAILABLE_MESSAGE };
  }

  logPipelineEvent("interpretation_ok", {
    chatHash,
    provider: provider?.name ?? null,
    latencyMs: interpretation.latencyMs ?? null,
    usage: interpretation.usage ?? null,
    actionCount: interpretation.plan.actions.length,
    actionTypes: interpretation.plan.actions.map((a) => a.type),
    confidences: interpretation.plan.actions.map((a) => a.confidence),
  });
