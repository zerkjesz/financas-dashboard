    ...
    async complete({ systemPrompt, userPrompt }) {
      ...
      try {
        ...
        if (res.status === 429) {
          ...
          throw new ProviderRateLimitError(retryAfter ? Number(retryAfter) : null);
        }
        if (!res.ok) {
...
//     só os números.
// ----------------------------------------------------------------------------
export function createGroqProvider({ apiKey = process.env.GROQ_API_KEY, model = process.env.GROQ_MODEL, timeoutMs = DEFAULT_TIMEOUT_MS, maxTokens = MAX_TOKENS, responseFormat } = {}) {
  return {
    name: "groq",
    isAvailable: () => Boolean(apiKey) && Boolean(model),
    async complete({ systemPrompt, userPrompt }) {
      ...
      try {
        ...
        if (!res.ok) {
          ...
          throw new ProviderRequestError(`Groq respondeu ${res.status}${bodyText ? `: ${bodyText.slice(0, 300)}` : ""}`, res.status);
        }
        const data = await res.json();
