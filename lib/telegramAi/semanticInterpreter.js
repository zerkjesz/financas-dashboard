...
import { ProviderUnavailableError, ProviderTimeoutError, ProviderRequestError } from "./llmProvider.js";
...
export async function interpretFinancialMessage({ text, now, accounts, cards, categories, conversationContext, financialContext, provider }) {
  ...
  try {
    ...
    return { kind: INTERPRETER_RESULT_KIND.MALFORMED_RESPONSE, detail: "resposta não é JSON válido", latencyMs };
  }

  const validation = parseAndValidatePlan(parsedJson);
  if (!validation.ok) {
    return { kind: INTERPRETER_RESULT_KIND.MALFORMED_RESPONSE, detail: validation.error, latencyMs };
  }

  return { kind: INTERPRETER_RESULT_KIND.OK, plan: validation.plan, latencyMs };
}
