...
export async function interpretFinancialMessage({ text, now, accounts, cards, categories, conversationContext, financialContext, provider }) {
  ...
  let raw, latencyMs, usage, rateLimitHeaders;
  try {
    const result = await provider.complete({ systemPrompt, userPrompt });
    raw = result.raw;
    latencyMs = result.latencyMs;
    usage = result.usage ?? null;
    rateLimitHeaders = result.rateLimitHeaders ?? null;
  } catch (err) {
    ...
    // Fase 7.0.3 (retomada) — `rateLimitHeaders` (sanitizado, só a allowlist
    // de nomes em llmProvider.js) é repassado pro caller poder registrar os
    // valores REAIS de quota (remaining/reset) num relatório, em vez de só
    // "levou um 429" — nunca inclui segredo, é a mesma allowlist de sempre.
    if (err instanceof ProviderRateLimitError) return { kind: INTERPRETER_RESULT_KIND.PROVIDER_RATE_LIMITED, retryAfterSeconds: err.retryAfterSeconds, rateLimitHeaders: err.rateLimitHeaders };
    // Fase 7.0.3 — achado real contra a Groq: o free tier pode rejeitar uma
    // requisição só de tamanho (413 "Request too large... tokens per minute
    // (TPM)"), sem nunca chegar a virar um 429 formal. `status` é repassado
    // pra quem chama (o acceptance harness) poder tratar isso com o MESMO
    // backoff de rate limit — nunca contar como falha de schema/modelo.
    if (err instanceof ProviderRequestError) return { kind: INTERPRETER_RESULT_KIND.PROVIDER_ERROR, detail: err.message, status: err.status, rateLimitHeaders: err.rateLimitHeaders };
  ...
  try {
    ...
    return { kind: INTERPRETER_RESULT_KIND.MALFORMED_RESPONSE, detail: "resposta não é JSON válido", latencyMs, usage, rateLimitHeaders };
  }

  const validation = parseAndValidatePlan(parsedJson);
  if (!validation.ok) {
    return { kind: INTERPRETER_RESULT_KIND.MALFORMED_RESPONSE, detail: validation.error, latencyMs, usage, rateLimitHeaders };
  }

  return { kind: INTERPRETER_RESULT_KIND.OK, plan: validation.plan, latencyMs, usage, rateLimitHeaders };
}
