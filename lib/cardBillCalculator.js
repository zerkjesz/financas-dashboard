...
export async function getCardBillView(card, cycleMonth) {
...
export async function listCardBillsView(cardId, { monthsBack = 2, monthsForward = 12 } = {}) {
  ...
  const bills = await Promise.all(months.map((month) => getCardBillView(card, month)));
