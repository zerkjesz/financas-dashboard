...
// ----------------------------------------------------------------------------
// Fase 8.0.1 — FATURA OBSERVADA É A VERDADE DA OBRIGAÇÃO.
//
// Quando existe uma CardBillReconciliation (snapshot REAL do app do banco) para
// ESTE cartão + ESTE ciclo, o total autoritativo da obrigação passa a ser
//   observedTotal + (lançamentos do cartão feitos DEPOIS da observação)
// e NÃO mais o CardBill.totalAmount guardado (que pode estar defasado — só é
// recomputado por escritas) nem o detalhe calculado (que tem lacuna conhecida).
// Tudo isto é derivado em memória, SEM escrita: nenhum GET/render/read-model
// precisa "abrir a tela de Cartões" pra ficar correto.
//
//  - detalhe conhecido (knownDetailTotal) = computeExpectedCardBillTotal (soma dos
//    Expense+Installment reais) — continua separado, nunca é apagado;
//  - lacuna (knownDetailGap) = total autoritativo − detalhe conhecido. Nunca vira
//    Expense/Purchase fabricado.
//  - "depois da observação" = criado depois da reconciliação (createdAt) E com data
//    econômica a partir do dia da observação. Item retroativo (data anterior ao dia
//    observado, lançado depois) NÃO soma: já está dentro do observado — reduz a lacuna
//    em vez de contar duas vezes.
//  - reconciliação de OUTRO ciclo/cartão nunca é aplicada (não sobrescreve fatura errada);
//    havendo várias pro mesmo ciclo, vale a mais recente.
//  - sem reconciliação: comportamento anterior, inalterado.
function utcDayStart(date) {
  const d = new Date(date);
  return new Date(Date.UTC(d.getUTCFullYear(), d.getUTCMonth(), d.getUTCDate()));
}

export async function findLatestReconciliationsByCycle(cardId, cycleMonths, { client = prisma } = {}) {
  const rows = await client.cardBillReconciliation.findMany({
    where: { cardId, cycleMonth: { in: cycleMonths } },
    orderBy: [{ createdAt: "desc" }, { id: "desc" }],
  });
  const byCycle = new Map();
  for (const r of rows) if (!byCycle.has(r.cycleMonth)) byCycle.set(r.cycleMonth, r);
  return byCycle;
}

export async function computeAdditionsSinceObservation(card, cycleMonth, reconciliation, { client = prisma } = {}) {
  const { start, end } = getCardBillPeriod(card, cycleMonth);
  const dayStart = utcDayStart(reconciliation.occurredAt);
  const from = dayStart > start ? dayStart : start;
  const [expenseSum, installmentSum] = await Promise.all([
    client.expense.aggregate({
      where: { cardId: card.id, occurredAt: { gte: from, lt: end }, createdAt: { gt: reconciliation.createdAt } },
      _sum: { amount: true },
    }),
    client.installment.aggregate({
      where: { billMonth: cycleMonth, purchase: { cardId: card.id, createdAt: { gt: reconciliation.createdAt }, purchasedAt: { gte: dayStart } } },
      _sum: { amount: true },
    }),
  ]);
  return addMoney(money(expenseSum._sum.amount), money(installmentSum._sum.amount));
...
// `bill`: qualquer objeto no formato de CardBill/visão (persistida ou projetada).
// `reconciliation`: undefined => busca; null => sabidamente não há; objeto => usa.
export async function applyObservedTotal(card, bill, { reconciliation, client = prisma } = {}) {
  const rec =
    reconciliation === undefined
      ? (await findLatestReconciliationsByCycle(card.id, [bill.cycleMonth], { client })).get(bill.cycleMonth) ?? null
      : reconciliation;
  if (!rec || rec.cardId !== card.id || rec.cycleMonth !== bill.cycleMonth) {
    return { ...bill, totalSource: bill.totalSource ?? (bill.isPersisted === false ? "calculated" : "stored") };
  }
  const [additions, knownDetailTotal] = await Promise.all([
    computeAdditionsSinceObservation(card, bill.cycleMonth, rec, { client }),
    computeExpectedCardBillTotal(card, bill.cycleMonth, { client }),
  ]);
  const observedTotal = money(rec.observedTotal);
  const totalAmount = addMoney(observedTotal, additions);
  return {
    ...bill,
    totalAmount,
    totalSource: "observed",
    observedTotal,
    observedAt: rec.occurredAt,
    observedReconciliationId: rec.id,
    additionsSinceObservation: additions,
    knownDetailTotal,
    knownDetailGap: subtractMoney(totalAmount, knownDetailTotal),
  };
}

export async function getCardBillView(card, cycleMonth, { now = new Date(), client = prisma, reconciliation } = {}) {
  const base = await getCardBillBaseView(card, cycleMonth, { now, client });
  return applyObservedTotal(card, base, { reconciliation, client });
}

async function getCardBillBaseView(card, cycleMonth, { now = new Date(), client = prisma } = {}) {
...
export async function listCardBillsView(cardId, { monthsBack = 2, monthsForward = 12, now = new Date(), client = prisma } = {}) {
  ...
  // Uma única query pras reconciliações do cartão (não uma por mês).
  const reconciliations = await findLatestReconciliationsByCycle(card.id, months, { client });
  const bills = await Promise.all(months.map((month) => getCardBillView(card, month, { now, client, reconciliation: reconciliations.get(month) ?? null })));
...
export async function payBill(cardBillId, { fromAccountId, amount, description, source, confidence, rawMessage, occurredAt }, { client = prisma } = {}) {
  ...
  // Fase 8.0.1 — o total que vale pra pagar é o AUTORITATIVO (observado, quando existe),
  // nunca só o CardBill.totalAmount guardado — senão não dá pra quitar a fatura real.
  const payCard = await client.card.findUnique({ where: { id: bill.cardId } });
  const effectiveTotal = (await applyObservedTotal(payCard, { ...bill, isPersisted: true }, { client })).totalAmount;
  ...
  const run = async (tx) => {
    const transfer = await tx.transfer.create({
      data: {
        ...
        // Fase 5.3D, item 26 — data ECONÔMICA (quando o pagamento aconteceu
        // de verdade), nunca confundida com quando a mensagem chegou.
        ...(occurredAt ? { occurredAt } : {}),
      },
    });
    const newPaidAmount = roundMoney(addMoney(alreadyPaid, amountMoney));
    const status = compareMoney(newPaidAmount, effectiveTotal) >= 0 ? "paid" : "partially_paid";
    const updated = await tx.cardBill.update({
      where: { id: bill.id },
      data: { status, paidAt: occurredAt || new Date(), paidAmount: newPaidAmount },
    });
...
export async function anticipateBill(cardBillId, { fromAccountId, amount, description, source, confidence, rawMessage, occurredAt }, { client = prisma } = {}) {
