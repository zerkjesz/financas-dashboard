...
export async function getCardBillView(card, cycleMonth, { now = new Date() } = {}) {
  const persisted = await prisma.cardBill.findUnique({ where: { cardId_cycleMonth: { cardId: card.id, cycleMonth } } });
  if (persisted) {
...
export async function listCardBillsView(cardId, { monthsBack = 2, monthsForward = 12, now = new Date() } = {}) {
  ...
  const bills = await Promise.all(months.map((month) => getCardBillView(card, month, { now })));
...
export async function resolveCurrentBillSafely(cardId) {
  ...
  const bill = await prisma.cardBill.findUnique({ where: { cardId_cycleMonth: { cardId, cycleMonth: cycleReference } } });
  if (!bill) {
    throw new Error(
