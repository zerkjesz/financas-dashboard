...
const INSTALLMENT_COUNTER_REGEX = /\b\d{1,2}\/\d{1,2}\b/g;
...
export function parseTransaction(rawMessage) {
  const textForAmount = rawMessage.replace(INSTALLMENT_COUNTER_REGEX, " ");
  ...
  return {
    ...
    paymentMethod: isInstallment ? "credit_card" : detectPaymentMethod(normalizedText),
