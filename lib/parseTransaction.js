...
const BILL_CATEGORY = "Fatura Cartão de Crédito";
...
// não é prefixo fixo tipo "parcela" — "fatura" pode aparecer em qualquer
// posição da frase ("paguei a fatura do cartão", "fatura cartão 500 pix").
// Exige "fatura" e "cartao" juntos pra não confundir com fatura de luz/água.
function isBillMessage(normalizedText) {
  return (
    includesWord(normalizedText, normalize("fatura")) &&
    includesWord(normalizedText, normalize("cartao"))
  );
}
...
export function parseTransaction(rawMessage) {
  ...
  const isBillPayment = !isInstallment && isBillMessage(normalizedText);
  ...
  let category = detectCategory(normalizedText);
  if (isInstallment) category = INSTALLMENT_CATEGORY;
  else if (isBillPayment) category = BILL_CATEGORY;
