...
export async function buildHomeModel({ now = new Date(), client = prisma } = {}) {
  ...
  for (const item of snapshot.currentObligations.breakdown) {
    ...
    if (!amount) continue;
    if (item.type === "Bill" && item.houseBill) { houseTotal = round2(houseTotal + amount); continue; } // agrupadas em UMA linha "Contas da casa"
  ...
  if (houseTotal > 0) parts.push({ kind: "casa", label: "Contas da casa", short: "Casa", amount: houseTotal, funded: false });
