...
export async function listGoals({ client = prisma } = {}) {
  const goals = await client.goal.findMany({ where: { isActive: true }, orderBy: { createdAt: "asc" } });
...
// Fase 7D.1 — não existia antes (app/api/goals/[id]/route.js fazia
// client.goal.update cru direto na rota); necessário pro "✏️ Editar meta" do
// menu determinístico, mesmo padrão de updateCommitmentDetails/
// updateContingencyAmount: uma função de domínio real, nunca Prisma cru no
// wizard. Só targetAmount por enquanto (o único campo que o menu determinístico
// precisa editar agora); mantém isActive/savedAmount intocados.
export async function updateGoalTargetAmount(goalId, targetAmount, { client = prisma } = {}) {
  const goal = await client.goal.findUnique({ where: { id: goalId } });
  if (!goal) throw new Error("Meta não encontrada");
  if (!goal.isActive) throw new Error("Meta inativa não pode ser editada");
  return client.goal.update({ where: { id: goalId }, data: { targetAmount: money(targetAmount) } });
}
