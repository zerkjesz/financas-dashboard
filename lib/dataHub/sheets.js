...
function periodRange(period, now = new Date()) {
  // Range literal (objeto Prisma {gte,lt/lte}) — usado por testes de
  // integração (Fase 6.0.1) pra exercitar planReplace/applyReplace de
  // verdade contra um escopo isolado e seguro, sem precisar de um 4º
  // período nomeado na UI (que continua só com os 3 de sempre).
...
const RAW_SHEETS = [
  ...
  },
  {
    ...
    async fetch({ period } = {}) {
      // Fase 8.0.1 — compromissos SEM prazo (dueDate null) não têm data pra cair num período: entram sempre.
      const rows = await prisma.confirmedCommitment.findMany({ where: periodRange(period) ? { OR: [{ dueDate: periodRange(period) }, { dueDate: null }] } : undefined, orderBy: { dueDate: "desc" } });
