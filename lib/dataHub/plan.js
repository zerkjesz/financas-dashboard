...
export async function planImport({ prisma, mode, datasets, rowsBySheet }) {
  ...
  for (const key of datasets) {
    ...
    // Fase 6.1 (Incident P0) — 1 query pra pré-carregar todo o índice de
    // match deste dataset, em vez de 1 query POR LINHA dentro do loop
    // abaixo (era a causa raiz do timeout de transação em produção — ver
    // lib/dataHub/adapters.js). Recalculado do zero a cada chamada de
    // planImport (inclusive a que roda DENTRO da transação do apply), então
    // a concorrência otimista continua vendo o estado real no momento certo.
    if (adapter.prepareMatchIndex) Object.assign(ctx, await adapter.prepareMatchIndex(prisma, rows));
