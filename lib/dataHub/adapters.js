...
function m(v) {
  return v == null || v === "" ? null : serializeMoney(money(v));
}
  ...
  return {
    ...
    async findMatch(prisma, row) {
      ...
      for (const f of naturalKeyFields) {
        ...
        where[f.field] = f.type === "date" ? normDate(v) : v;
      }
      const candidates = await prisma[model].findMany({ where });
