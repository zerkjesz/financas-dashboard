...
export async function applyReplace(prisma, { id: batchId, datasets, period, rowsBySheet, fileName, fileHash }) {
  return prisma.$transaction(async (tx) => {
    const { perDataset, addPlan } = await planReplace({ prisma: tx, datasets, period, rowsBySheet });
    ...
    return { counts, preimages, plan: { perDataset, addPlan }, undoDeadline };
  });
}
