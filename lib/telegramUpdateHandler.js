...
async function detectReadIntent(update, chatId) {
  ...
  return READ_INTENTS.has(intent) ? intent : null;
}
...
export async function handleTelegramUpdate(update) {
  ...
  // segunda fórmula.
  const readIntent = await detectReadIntent(update, chatId);
  if (readIntent) {
    const reply = await handleReadIntent(readIntent);
    await sendMessage(chatId, reply);
    return { status: "processed_read" };
  }
