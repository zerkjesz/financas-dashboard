...
export async function dispatchUpdate(update, chatId, { client, outbox }) {
  ...
  //
  //   false (default) -> pipeline NUNCA é chamado; comportamento é 100% o
  //   parser legado, exatamente como se a Fase 7.0 não existisse.
  //
  //   true -> pipeline é SEMPRE chamado (quando a conversa não pertence ao
  //   parser antigo — wizard ativo, ou um PendingBotMessage estrangeiro).
  //   QUALQUER resultado que não seja REPLY/SILENT (ou seja, NO_PROVIDER —
  //   sem ANTHROPIC_API_KEY, timeout, erro HTTP, resposta malformada, schema
  //   inválido, todos já reduzidos a REPLY ou NO_PROVIDER dentro do
  //   pipeline) termina AQUI, com a mensagem de segurança fixa — NUNCA cai
  //   pro parser legado. Resolve a contradição da Fase 7.0 (onde "sem
  //   provider" e "provider falhou" tinham comportamentos diferentes).
