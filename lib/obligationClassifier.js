...
export const OBLIGATION_CLASS = Object.freeze({
  ...
  CURRENT_HORIZON_OBLIGATION: "CURRENT_HORIZON_OBLIGATION", // vence até a próxima renda (== DUE_BEFORE_NEXT_INCOME).
  // Fase 5.2B — obrigação real, sem data de calendário conhecida, mas atrelada
  // ao TIMING da próxima renda ("paga geralmente logo depois do salário"). NÃO
  // reduz freeMoney atual (mesmo tratamento de FUTURE_OBLIGATION nesse
  // sentido), mas É somada em getNextIncomeCommitment (lib/freeMoney.js) — só
  // ela existe pra permitir essa distinção sem inventar uma dueDate falsa.
  NEXT_INCOME_WINDOW_COMMITMENT: "NEXT_INCOME_WINDOW_COMMITMENT",
...
export function classifyBill(bill, { nextIncomeDate }) {
  ...
  // "pending"
  // Fase 9.1 — sem vencimento informado: nunca inventa data; sem data não dá pra afirmar que vence antes da renda.
  if (bill.dueDate == null) return OBLIGATION_CLASS.FUTURE_OBLIGATION;
  return bill.dueDate <= nextIncomeDate ? OBLIGATION_CLASS.CURRENT_HORIZON_OBLIGATION : OBLIGATION_CLASS.FUTURE_OBLIGATION;
}

// ============================================================================
// ExternalInstallment (item 9 / Fase 5.2B)
// ============================================================================
//
// `dueTiming` vem do PLAN (fonte única — nunca duplicado na installment, ver
// schema.prisma) — default "CALENDAR_DATE" preserva o comportamento anterior
// inalterado pra todo plano já existente/futuro com data real conhecida.
//
// `isFirstUnpaidOfPlan`: quem chama (lib/freeMoney.js) já resolveu, olhando
// TODAS as irmãs PENDING do mesmo plano ordenadas por `number`, se esta é a
// PRÓXIMA a vencer — nunca decidido aqui (função pura, sem acesso a outras
// rows). Crítico (item 5 do pedido): um plano AFTER_NEXT_INCOME com N parcelas
// pendentes tem só a primeira delas como NEXT_INCOME_WINDOW_COMMITMENT — as
// demais continuam FUTURE_OBLIGATION até vencerem a vez (a anterior ser paga).
// Nunca soma as N de uma vez só por estarem no mesmo plano sem dueDate.
export function classifyExternalInstallment(installment, { nextIncomeDate, dueTiming = "CALENDAR_DATE", isFirstUnpaidOfPlan = false } = {}) {
  if (installment.status === "PAID") return OBLIGATION_CLASS.SETTLED;
  if (dueTiming === "AFTER_NEXT_INCOME") {
    return isFirstUnpaidOfPlan ? OBLIGATION_CLASS.NEXT_INCOME_WINDOW_COMMITMENT : OBLIGATION_CLASS.FUTURE_OBLIGATION;
  }
...
export function classifyConfirmedCommitment(commitment, { nextIncomeDate }) {
  ...
  // Fase 8.0.1 — SEM PRAZO (dueDate null): nunca inventa uma data pra comparar. Sem data
  // não dá pra afirmar que vence antes da próxima renda, então:
  //   FUNDED    -> CURRENT_HORIZON (regra B: earmark já existe — o dinheiro NÃO é livre);
  //   CONFIRMED -> FUTURE_OBLIGATION (regra C: nada indica vencimento antes da próxima renda).
  if (commitment.dueDate == null) {
    return commitment.status === "FUNDED" ? OBLIGATION_CLASS.CURRENT_HORIZON_OBLIGATION : OBLIGATION_CLASS.FUTURE_OBLIGATION;
  }
