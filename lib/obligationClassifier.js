...
import { getCardBillPeriod } from "./cardCycle.js";
...
// ============================================================================
//
// INCURRED_LIABILITY quando: saldo restante (totalAmount - paidAmount) > 0 E o
// ciclo dessa fatura já COMEÇOU (período.start <= now) — cobre tanto a fatura já
// fechada quanto a fatura do ciclo corrente ainda aberta (gasto já feito = dívida
// já incorrida, mesmo antes do fechamento formal). Uma CardBill materializada
// pra um ciclo que ainda nem começou (ex: listBillsForCard olha 12 meses à
// frente) é FUTURE_OBLIGATION — não sequestra freeMoney de hoje.
//
...
export function classifyCardBill(bill, card, { now = new Date() } = {}) {
  ...
  const period = getCardBillPeriod(card, bill.cycleMonth);
  const cycleAlreadyStarted = period.start <= now;
  return cycleAlreadyStarted ? OBLIGATION_CLASS.INCURRED_LIABILITY : OBLIGATION_CLASS.FUTURE_OBLIGATION;
}
