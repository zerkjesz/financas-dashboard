...
// pessoa/credor direto — eletrônico, viagem, imóvel de terceiro etc).
// Diferente de lib/installments.js (Purchase/Installment, sempre presa a um
// Card).
...
//
// Fase 9.1 — PROGRESSO REAL. Um plano cadastrado depois de já ter parcelas pagas fora do Norte
// (alreadyPaidCount, Fase 5.2C) só tem linhas a partir da posição real (ex.: impressora 3D: 10
// parcelas, 6 pagas antes => linhas #7..#10). Contar só as linhas dava "0/4" — falso. Passando
// `installmentCount` (o total do PLANO), o progresso é: pagas = total - pendentes (histórico
// anterior ao Norte + linhas PAID), próxima = menor número PENDING. Sem `installmentCount`,
// comportamento anterior inalterado (só conta linhas).
export function computePlanProgress(installments, { installmentCount } = {}) {
  const paidRows = installments.filter((i) => i.status === "PAID").length;
  if (installmentCount == null) {
    return { paidCount: paidRows, totalCount: installments.length, isFullyPaid: installments.length > 0 && paidRows === installments.length };
  }
  const pending = installments.filter((i) => i.status === "PENDING");
  const paidCount = Math.max(0, installmentCount - pending.length);
  return {
    paidCount,
    totalCount: installmentCount,
    pendingCount: pending.length,
    paidRows,
    historicalPaidCount: Math.max(0, paidCount - paidRows),
    nextNumber: pending.length ? Math.min(...pending.map((i) => i.number)) : null,
    isFullyPaid: installmentCount > 0 && pending.length === 0,
  };
}
...
export async function listExternalInstallmentPlans({ status, client = prisma } = {}) {
  ...
  return plans.map((plan) => ({
    ...
    progress: computePlanProgress(plan.installments, { installmentCount: plan.installmentCount }),
  }));
}
