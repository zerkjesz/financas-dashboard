...
export async function computeCardTotalLimit(cardId) {
  ...
  const lastLimitChange = await prisma.cardLimitUpdate.findFirst({
    where: { cardId, newTotalLimit: { not: null } },
...
export async function listCardsWithLimits() {
  ...
  return Promise.all(
    cards.map(async (card) => ({
      ...card,
      totalLimit: await computeCardTotalLimit(card.id),
      usedLimit: await computeCardUsedLimit(card.id),
      availableLimit: await computeCardAvailableLimit(card.id),
      amountAnticipated: await computeAmountAnticipated(card.id),
    }))
  );
}
