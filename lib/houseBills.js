...
// ============================================================================
// Fase 9.1.1 — CONTAS DA CASA NA VERDADE FINANCEIRA (committed / freeMoney / safeToSpend /
// simulador / projeção). Só LEITURA. Regras:
...
//   * só a competência CORRENTE  -> o mês em curso inteiro precisa ser coberto (é o que a página
//                                   Compromissos mostra); competências futuras entram quando virarem;
//   * cada visita da faxina é uma obrigação própria (0/2 => R$260, 1/2 => R$130, 2/2 => R$0);
//   * `handled` = chaves regra:competência:parte já representadas aqui — o caminho legado de Bill
//     pendente as ignora (sem double count).
// ============================================================================
export function houseBillDescription(inst) {
  return inst.partsTotal > 1 ? `${inst.name} — visita ${inst.part}/${inst.partsTotal}` : inst.name;
}

const toUnpriced = (inst) => ({
  key: inst.key,
  ruleId: inst.ruleId,
...
// Obrigações de contas da casa que SEQUESTRAM caixa: SOMENTE a competência CORRENTE (o mês em curso —
// exatamente o que a página Compromissos lista). Competências futuras (ex.: aluguel do mês que vem) entram
// quando a competência virar; nunca são projetadas aqui. `horizonEnd` é aceito por compatibilidade de
// assinatura com os chamadores do motor e não amplia o escopo.
export async function getHouseBillObligations({ now = new Date(), client = prisma } = {}) {
  const current = currentMonthKey(now);
  const instances = await listHouseBillInstances({ cycleMonth: current, now, client });
  const handled = new Set(instances.map((i) => i.key));
  const items = [];
  const unpriced = [];
  for (const inst of instances) {
    if (inst.status !== "PENDING") continue; // PAID nunca é obrigação
    if (inst.awaitingValue || inst.partAmount == null) unpriced.push(toUnpriced(inst));
    else items.push(toObligation(inst));
  }
  return { items, unpriced, handled, months: [current] };
}
