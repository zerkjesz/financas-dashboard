...
// Usado pro token de SESSÃO ({ iat, exp }, sem dado sensível). Até a Fase
// 5.5 também era usado pelo contador de rate-limit do login (cookie
// assinado, lib/auth/rateLimit.js) — removido nessa fase em favor de
// enforcement autoritativo em Postgres (lib/auth/rateLimitDb.js), que
// reaproveita o mesmo SESSION_SECRET só pra derivar a chave HMAC (não pra
// assinar/verificar payload — por isso não usa mais este módulo).
//
