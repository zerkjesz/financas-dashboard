...
// ============================================================================
const INTERROGATIVE_START_RE = /^\s*(quanto|quantos?|quanta|qual|quais|como|quando|cad[eê]|o que)\b/i;

function isInterrogative(text) {
  return text.includes("?") || INTERROGATIVE_START_RE.test(text);
}
...
const READ_SUMMARY_RE = /\bresumo\b|\bcomo (eu )?(t[oô]|estou)\b|\bsitua[cç][ãa]o financeira\b|\bcomo (estao|est[ãa]o) minhas financas\b/i;
const READ_FREE_MONEY_RE = /\blivre\b|\bposso gastar\b|\bseguro.*gastar\b|\bgastar.*seguro\b/i;
const READ_NEXT_INCOME_RE = /\bsal[aá]rio\b|\brenda\b/i;
const READ_EXTERNAL_INSTALLMENTS_RE = /\bparcelas?\b/i;
const READ_VA_RE = /\bvale\b|\bcaju\b|\bva\b/i;
const READ_CARD_RE = /\bfatura\b|\blimite\b|\bcart[ãa]o\b/i;
const READ_BALANCE_RE = /\bsaldo\b|\btenho\b/i;

function classifyReadIntent(rawMessage) {
  const normalized = normalize(rawMessage);

  if (READ_SUMMARY_RE.test(rawMessage)) return { intent: "read_summary" };

  if (!isInterrogative(rawMessage)) return null; // tudo abaixo exige pergunta explícita.
