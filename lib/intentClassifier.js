...
// Fase 5.6.2 — a presença de um NÚMERO no texto não é, por si só, evidência
// de intenção financeira. Antes do fallback `expense`, a mensagem precisa
// carregar ao menos UM sinal positivo de que é um lançamento: um verbo
// financeiro, um substantivo financeiro, um marcador de moeda, ou um método
// de pagamento. Sem nenhum sinal, a mensagem NÃO é um lançamento — mesmo
// contendo dígitos (token de verificação, código de pedido, data, hora,
// versão, placa, telefone, UUID…). Genérico de propósito: nada aqui
// menciona nenhum challenge específico.
const FINANCIAL_INTENT_EVIDENCE_RE =
  /\b(gastei|gasto|gastar|gastando|paguei|pago|paga|pagar|pagando|comprei|compra|comprar|comprando|custou|custa|custo|torrei|desembolsei|parcelei|parcelar|fiado|fatura|boleto|pix|d[eé]bito|cr[eé]dito|dinheiro|reais|real)\b/i;
// "R$" cru: normalize() remove o "$", então testado no texto original.
const CURRENCY_SYMBOL_RE = /r\$\s*\d|\d+\s*(reais|real)\b/i;

function hasFinancialIntentEvidence(rawMessage, normalizedText) {
  if (CURRENCY_SYMBOL_RE.test(rawMessage)) return true;
  if (FINANCIAL_INTENT_EVIDENCE_RE.test(normalizedText)) return true;
  // número com contexto de moeda explícito ("valor 200", "total 50")
  const { candidates } = extractAmount(rawMessage);
  return candidates.some((c) => c.hasCurrencyContext);
}
...
export function classifyIntent(rawMessage) {
  ...
  // Fase 5.6.2 — INTENT ANTES DE AMOUNT. Chegou aqui sem casar nenhuma regra
  // específica: só vira `expense` se a mensagem tiver evidência de intenção
  // financeira. Caso contrário NÃO é um lançamento (texto arbitrário que por
  // acaso contém dígitos) — nunca cria candidato/PendingBotMessage.
  const payVerbCandidate = PAY_VERB_RE.test(text);
  if (!payVerbCandidate && !hasFinancialIntentEvidence(text, normalized)) {
    return { intent: "no_financial_intent" };
  }

  return { intent: "expense", payVerbCandidate };
}
