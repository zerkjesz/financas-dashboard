...
export async function getProtectedMoneyBreakdown({ unrestrictedAccountIds } = {}) {
  const reserves = await prisma.reserve.findMany({ where: { isActive: true } });
  const breakdown = [];
  for (const reserve of reserves) {
    const currentAmount = await getReserveBalance(reserve.id);
...
export async function getProtectedMoney({ unrestrictedAccountIds } = {}) {
  const breakdown = await getProtectedMoneyBreakdown({ unrestrictedAccountIds });
...
async function classifyAllObligations({ now = new Date(), nextIncomeDate } = {}) {
  ...
  for (const card of cards) {
    const bills = await prisma.cardBill.findMany({ where: { cardId: card.id } });
    for (const bill of bills) {
      const cls = classifyCardBill(bill, card, { now });
  ...
  const [bills, externalInstallments, commitments] = await Promise.all([
    prisma.bill.findMany({ where: { status: { in: ["pending", "overdue"] } } }),
    prisma.externalInstallment.findMany({ where: { status: "PENDING" } }),
    prisma.confirmedCommitment.findMany({ where: { status: { in: ["CONFIRMED", "FUNDED"] } } }),
  ]);
  ...
  for (const installment of externalInstallments) {
    ...
    push(cls, { type: "ExternalInstallment", id: installment.id, planId: installment.planId, amount: money(installment.amount), dueDate: installment.dueDate });
  }
...
export async function getObligationsBreakdown({ now = new Date(), nextIncomeDate } = {}) {
  return classifyAllObligations({ now, nextIncomeDate });
}
...
export async function getIncurredLiabilities({ now = new Date(), nextIncomeDate = now } = {}) {
  const buckets = await classifyAllObligations({ now, nextIncomeDate });
...
export async function getCurrentHorizonObligations({ now = new Date(), nextIncomeDate } = {}) {
  const buckets = await classifyAllObligations({ now, nextIncomeDate });
...
// ============================================================================
export async function getFutureObligations({ now = new Date(), nextIncomeDate } = {}) {
  const buckets = await classifyAllObligations({ now, nextIncomeDate });
...
export async function getContingencyExposure() {
...
export async function computeFreeMoney({ now = new Date(), accounts, nextIncomeDate } = {}) {
  if (!nextIncomeDate) throw new Error("nextIncomeDate é obrigatório");
...
// ============================================================================
//
// Janela: (nextIncome.expectedDate, ocorrência seguinte da mesma regra] — ou
// seja, o PRÓXIMO ciclo de pagamento, não o horizonte atual (que já é coberto
// por currentHorizonObligations). Deliberadamente NÃO reusa
// classifyBill/classifyExternalInstallment/classifyConfirmedCommitment (que
// respondem "isso afeta freeMoney AGORA?", uma pergunta de corte único) — aqui
// a pergunta é "o que vence dentro desta JANELA futura específica", por isso é
// uma soma por intervalo de data direta. Isto NÃO é dupla contagem: é outro
// indicador (não outra subtração do caixa) — dívida que já reduz freeMoney
// HOJE pode aparecer aqui de novo se o vencimento dela cair dentro da janela,
// exatamente como o pedido especifica (item 18: "incluir dívida já incorrida
// com dueDate dentro desse intervalo, mesmo que ela já esteja reduzindo
// freeMoney atual").
async function sumObligationsInWindow(periodStart, periodEnd) {
  const inWindow = (date) => date > periodStart && date <= periodEnd;

  const [cardBills, bills, externalInstallments, commitments] = await Promise.all([
    prisma.cardBill.findMany({ where: { dueAt: { gt: periodStart, lte: periodEnd } } }),
    prisma.bill.findMany({ where: { status: { in: ["pending", "overdue"] }, dueDate: { gt: periodStart, lte: periodEnd } } }),
    prisma.externalInstallment.findMany({ where: { status: "PENDING", dueDate: { gt: periodStart, lte: periodEnd } } }),
    prisma.confirmedCommitment.findMany({ where: { status: { in: ["CONFIRMED", "FUNDED"] }, dueDate: { gt: periodStart, lte: periodEnd } } }),
  ]);

  let total = ZERO;
  for (const bill of cardBills) {
    if (!inWindow(bill.dueAt)) continue;
    total = addMoney(total, subtractMoney(money(bill.totalAmount), money(bill.paidAmount ?? 0)));
  }
...
export async function getNextIncomeCommitment({ nextIncome } = {}) {
  ...
  const committedAmount = await sumObligationsInWindow(periodStart, periodEnd);
