...
async function classifyAllObligations({ now = new Date(), nextIncomeDate, client = prisma } = {}) {
  ...
  const [bills, externalInstallments, commitments] = await Promise.all([
    ...
    client.externalInstallment.findMany({ where: { status: "PENDING" } }),
...
async function sumObligationsInWindow(periodStart, periodEnd, client = prisma) {
...
export async function getNextIncomeCommitment({ nextIncome, client = prisma } = {}) {
  ...
  const committedAmount = await sumObligationsInWindow(periodStart, periodEnd, client);
