...
export function resolveCurrentRelevantCardBillId(bills) {
  ...
  return unsettled.length > 0 ? unsettled[0].id : null;
}
...
async function classifyAllObligations({ now = new Date(), nextIncomeDate } = {}) {
  ...
  // resolveCurrentRelevantCardBillId acima); mantido no parâmetro da função
  // por estabilidade de API (outros chamadores já passam) e porque
  ...
  for (const card of cards) {
    ...
    const currentRelevantId = resolveCurrentRelevantCardBillId(bills);
    for (const bill of bills) {
      const cls = classifyCardBill(bill, { isCurrentRelevant: bill.id === currentRelevantId });
...
async function sumObligationsInWindow(periodStart, periodEnd) {
  ...
  const [cardBills, bills, externalInstallments, commitments] = await Promise.all([
    prisma.cardBill.findMany({ where: { dueAt: { gte: periodStart, lt: periodEnd } } }),
    prisma.bill.findMany({ where: { status: { in: ["pending", "overdue"] }, dueDate: { gte: periodStart, lt: periodEnd } } }),
    prisma.externalInstallment.findMany({ where: { status: "PENDING", dueDate: { gte: periodStart, lt: periodEnd } } }),
    prisma.confirmedCommitment.findMany({ where: { status: { in: ["CONFIRMED", "FUNDED"] }, dueDate: { gte: periodStart, lt: periodEnd } } }),
  ]);
