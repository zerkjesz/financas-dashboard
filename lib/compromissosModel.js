...
import { listHouseBillInstances, AMOUNT_KIND, houseBillDescription, monthKeysBetween, monthKeyOfDate } from "./houseBills.js";
import { getNextIncomeInfo } from "./incomeHorizon.js";
import { computeCurrentObligationHorizonEnd } from "./financialEngine.js";
...
export async function buildCommitmentsModel({ now = new Date(), client = prisma } = {}) {
  ...
  for (const [ruleId, parts] of byRule) {
    ...
    items.push({
      ...
      casa: {
        ...
        // Fase 9.1.1 — cada visita/conta paga da competência, com o token de "Reabrir" (stale guard).
        paidParts: [...paidParts].sort((a, b) => a.part - b.part).map((p) => ({
          part: p.part,
          amount: num(p.partAmount),
          when: whenLabel(p.paidAt, today),
          sourceName: p.paidAccountName ?? null,
          withoutExpense: !!p.paidWithoutExpense,
          undo: p.billId ? { kind: "house", id: p.billId, expectedUpdatedAt: p.billUpdatedAt.toISOString() } : null,
        })),
      },
  ...
  // ---------------------------------------------------------------- antes da próxima renda (Fase 9.1.2)
  // COMPETÊNCIA organiza; HORIZONTE decide. Uma conta de OUTRA competência que vence até a próxima renda
  // já está no "comprometido" (é a MESMA regra do motor: dueDate <= horizonte efetivo). Aqui ela fica
  // localizável na página, com o mesmo cartão/pagamento das contas da casa.
  const nextIncome = await getNextIncomeInfo({ now, client });
  const horizonEnd = computeCurrentObligationHorizonEnd(nextIncome, now);
  const horizonMonths = monthKeysBetween(monthKey, monthKeyOfDate(horizonEnd) < monthKey ? monthKey : monthKeyOfDate(horizonEnd)).filter((m) => m !== monthKey);
  const MON = ["jan", "fev", "mar", "abr", "mai", "jun", "jul", "ago", "set", "out", "nov", "dez"];
  const dm = (d) => `${String(d.getUTCDate()).padStart(2, "0")}/${String(d.getUTCMonth() + 1).padStart(2, "0")}`;
  const incomeLabel = nextIncome.expectedDate ? dm(nextIncome.expectedDate) : null;
  const beforeItems = [];
  for (const m of horizonMonths) {
    for (const inst of await listHouseBillInstances({ cycleMonth: m, now, client })) {
      if (inst.status !== "PENDING" || inst.dueDate == null || inst.dueDate > horizonEnd) continue;
      beforeItems.push({
        id: `casa-antes:${inst.key}`,
        kind: "casa",
        kindLabel: `Conta da casa · ${String(inst.dueDate.getUTCDate()).padStart(2, "0")} ${MON[inst.dueDate.getUTCMonth()]}`,
        beforeNextIncome: true,
        name: houseBillDescription(inst),
        to: null,
        note: null,
        state: "pending",
        awaitingValue: !!inst.awaitingValue,
        overdue: false,
        value: num(inst.partAmount),
        casa: {
          ruleId: inst.ruleId,
          ...
          partsTotal: 1,
          partsPaid: 0,
          amountKind: inst.amountKind,
          cadence: inst.cadence,
          defaultAccountId: inst.defaultAccountId,
          dueDay: inst.dueDay,
          dueDate: inst.dueDate.toISOString(),
          referenceMin: num(inst.referenceMin),
          referenceMax: num(inst.referenceMax),
          monthlyTotal: num(inst.partAmount) ?? 0,
          remainingAmount: num(inst.partAmount) ?? 0,
          lastPaid: null,
          paidParts: [],
          beforeIncomeLabel: incomeLabel,
        },
        pay: { kind: "house", ruleId: inst.ruleId, cycleMonth: inst.cycleMonth, part: inst.part },
        undo: null,
        done: null,
      });
    }
  }
  beforeItems.sort((a, b) => new Date(a.casa.dueDate) - new Date(b.casa.dueDate));
  const beforeNextIncome = {
    nextIncomeDate: nextIncome.expectedDate ? nextIncome.expectedDate.toISOString() : null,
    nextIncomeLabel: incomeLabel,
    horizonEnd: horizonEnd.toISOString(),
    status: nextIncome.status,
    knownTotal: Math.round(beforeItems.reduce((a, i) => a + (i.value ?? 0), 0) * 100) / 100,
    unpricedCount: beforeItems.filter((i) => i.awaitingValue).length,
    items: beforeItems,
  };
