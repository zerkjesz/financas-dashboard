...
| Variáveis de ambiente de produção (Vercel) | CONFIGURATION | **VERIFICADO + PARCIALMENTE PROVISIONADO (5.5.2)**. Já em produção: `DATABASE_URL`, `TELEGRAM_TOKEN` (49d) + `DIRECT_URL` (novo — Neon endpoint `production` unpooled, verificado), `DATABASE_ENV=production`, `APP_TIMEZONE=America/Sao_Paulo` (novos — valores de contrato/deriváveis, setados via `vercel env add`). `AUTH_DEV_BYPASS` **ausente = SEGURO**. **AINDA AUSENTES** (segredos humanos, `HUMAN_ACTION_REQUIRED` — não inventar): `SESSION_SECRET`, `DASHBOARD_PASSWORD_HASH`, `TELEGRAM_WEBHOOK_SECRET`, `TELEGRAM_ALLOWED_USER_ID`. Nenhum efeito no deployment atual (env só vale no próximo build). |
