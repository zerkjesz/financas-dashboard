...
| Variáveis de ambiente de produção (Vercel) | CONFIGURATION | **PROVISIONADO (5.5.2 + 5.5.3)** — todas as 8 vars type `sensitive`, sem efeito no deployment atual (env só vale no próximo build). **PRESENTES**: `DATABASE_URL` (49d), `TELEGRAM_TOKEN` (49d), `DIRECT_URL` (Neon endpoint `production` unpooled — mesmo string usado no audit read-only da 5.5.2), `DATABASE_ENV=production`, `APP_TIMEZONE=America/Sao_Paulo`, **`SESSION_SECRET`** (5.5.3 — `crypto.randomBytes(48)` → 96 hex), **`DASHBOARD_PASSWORD_HASH`** (5.5.3 — `scrypt:16384:8:1:…` via `lib/auth/password.js:hashPassword`, round-trip verificado; passphrase no macOS login keychain, serviço `norte-dashboard-prod`), **`TELEGRAM_WEBHOOK_SECRET`** (5.5.3 — `crypto.randomBytes(32)` → 64 hex, charset válido de `secret_token`). `AUTH_DEV_BYPASS` **ausente = SEGURO**. **ÚNICO AUSENTE**: `TELEGRAM_ALLOWED_USER_ID` — `HUMAN_ACTION_REQUIRED_FINAL` (é o `from.id` numérico de uma pessoa real; não é segredo gerável; nenhuma fonte acessível tem valor real — `.env` local só tem o placeholder `123456789`). **Não bloqueia o deploy web/auth** (o bot é fail-closed até ser setado). |
