...
| `OWNER_CHAT_ID` | chat_id do Telegram do único usuário autorizado a interagir com o bot | Sim, em produção | Não é segredo por si (é um ID), mas não deve ser público | webhook route + `bot/telegram-bot.js` |
...
4. Descobrir `OWNER_CHAT_ID`: mandar qualquer mensagem pro bot uma vez e olhar o campo `chat.id` do update recebido (ou usar um bot utilitário tipo @userinfobot).
